...
const Enquiry = () => {
  ...
  const fetchEnquiry = async () => {
    try {
      ...
      setEnquiry(response.data.contactUs);
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${base_url}/contactus/delete/${id}`);
      fetchEnquiry();
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div className=" min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-xl font-semibold">Enquiry Field</h1>
          <p className="text-gray-500 text-sm">
            Check, Who is trying to connect with you.....
          </p>
        </div>
      </div>
